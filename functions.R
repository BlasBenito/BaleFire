...
plotDiagram <- function(
  ...
  x.limits <- scale_x_continuous(
    expand = c(0, 0),
    limits = c(
      0, ceiling(max(pollen.data$age))
      ),
    breaks = seq(0, ceiling(max(pollen.data$age)), by = 1)
    )
...
interpolateDatasets <- function(
  ...
  age.ranges <- sapply(
    datasets.list,
    FUN = function(x) range(x[, age.column.name])
  )
  # min of maximum ages
  ...
  datasets.list <- lapply(
    datasets.list, function(x) x[x[, age.column.name] >= min.age & x[, age.column.name] <= max.age, ]
  )
  ...
  for(dataset.to.interpolate in names(datasets.list)) {
    ...
    for (column.to.interpolate in colnames.temp) {
      ...
      for (span in span.values) {
        ...
        if (
          cor(
            interpolation.function$fitted,
            temp[, column.to.interpolate]
          ) >= 0.99999999
        ) {
          break
        }
      ...
      print(
        paste("Correlation between observed and interpolated data = ",
          cor(
            interpolation.function$fitted,
            temp[, column.to.interpolate]
          ),
...
forwardLags <- function(
  ...
  lag.data <- data.frame(
    ericaceae.par = double(),
    charcoal.acc.rate = double(),
    lag = integer()
    )
  ...
  for (erica.case in nrow(erica):1) {
    ...
    if (erica.case.age.plus.lags < min(char.interpolated$age)) {
      break
    }

    # get from char.interpolated the lines with age > age.erica && age <= age.erica + lags
    char.temp <- char.interpolated[
      which(
        char.interpolated$age < erica.case.age &
          char.interpolated$age >= erica.case.age.plus.lags
        ),
      "charcoal.acc.rate"
      ]

    # put the data together (NOTE: lags have to be added in reverse)
    char.temp <- data.frame(
      ericaceae.par = erica.value,
      charcoal.acc.rate = char.temp,
      lag = rev(lags)
      )
...
backwardLags <- function(
  ...
  for (erica.case in 1:nrow(erica)) {
    ...
    # if beyond maximum age
    if (erica.case.age.plus.lags > max(char.interpolated$age)) {
      break
    }
...
modelLagData <- function(
  ...
  # computing R2 per lag
  for (lag in lags) {
    results.list.R2[[lag]] <- cor(lagged.data[lagged.data$lag == lag, response], predict(results.list[[lag]]))^2
  }

  # gathering coefficients
  results.list.coef <- lapply(results.list, function(x) {
    summary(x)$tTable[2, 1]
  })

  # gathering the standard error of the coefficients
  results.list.coef.se <- lapply(results.list, function(x) {
    summary(x)$tTable[2, 2]
  })

  # gathering p-value of coefficients
  results.list.pvalue <- lapply(results.list, function(x) {
    summary(x)$tTable[2, 4]
  })
  ...
  # adding the errors
  output.df.long$lower <- c(
    output.df.se$lower,
    output.df.long[output.df.long$variable == "p-value", "value"],
    output.df.long[output.df.long$variable == "R2", "value"]
    )

  output.df.long$upper <- c(
    output.df.se$upper,
    output.df.long[output.df.long$variable == "p-value", "value"],
    output.df.long[output.df.long$variable == "R2", "value"]
    )
...
modelRandomLagData <- function(
  ...
  null.model.df <- data.frame(
    lag = results[[1]]$lag,
    variable = results[[1]]$variable,
...
plotModelOutput <- function(
  forward.results,
  forward.results.random,
  backward.results,
  backward.results.random,
  filename,
  width,
  height,
  title.size,
  ...
  # axes limits
  max.lag <- max(
    c(forward.results$lag,
      backward.results$lag
      )
    )
  max.coefficient <- round(
    max(
      c(forward.results[forward.results$variable == "Coefficient", "value"],
        backward.results[backward.results$variable == "Coefficient", "value"],
        forward.results.random[forward.results.random$variable == "Coefficient", "upper"],
        backward.results.random[backward.results.random$variable == "Coefficient", "upper"])
      ) + 0.1,
    1
    )
  min.coefficient <- round(
    min(
      c(
        forward.results[forward.results$variable == "Coefficient", "value"],
        backward.results[backward.results$variable == "Coefficient", "value"],
        forward.results.random[forward.results.random$variable == "Coefficient", "lower"],
        backward.results.random[backward.results.random$variable == "Coefficient", "lower"])
      ) - 0.1,
    1
    )
  max.R2 <- round(
    max(
      c(
        forward.results[forward.results$variable == "R2", "value"],
        backward.results[backward.results$variable == "R2", "value"])
      ),
    1
    )
  ...
  # BACKWARD PLOT
  backward.plot.coefficient <- ggplot(
    data = subset(backward.results, variable == "Coefficient"),
    aes(
      x = lag,
      y = value
      )
    ...
    geom_ribbon(
      data = subset(backward.results.random, variable == "Coefficient"),
      aes(
        ymin = lower,
        ymax = upper
        ),
      alpha = 0.3,
      fill = viridis.colors[10]
      ) +
    geom_line(
      data = subset(backward.results.random, variable == "Coefficient"),
      aes(
    ...
    scale_x_reverse(
      limits = c(max.lag, 10),
      breaks = c(10, seq(100, max.lag, by = 100))
      ) +
    theme(
  ...
  forward.plot.coefficient <- ggplot(
    data = subset(forward.results, variable == "Coefficient"),
    aes(
      x = lag,
      y = value,
      group = variable
    ...
    geom_ribbon(
      data = subset(forward.results.random, variable == "Coefficient"),
      aes(
        ymin = lower,
        ymax = upper
        ),
  ...
  backward.plot.R2 <- ggplot(
    data = subset(backward.results, variable == "R2"),
    aes(
      x = lag,
      y = value,
      group = variable)
    ...
    geom_ribbon(
      data = subset(backward.results.random, variable == "R2"),
      aes(
        ymin = lower,
        ymax = upper
        ),
  ...
  forward.plot.R2 <- ggplot(
    data = subset(forward.results, variable == "R2"),
    aes(
      x = lag,
      y = value,
      group = variable)
    ...
    geom_ribbon(
      data = subset(forward.results.random, variable == "R2"),
      aes(
        ymin = lower,
        ymax = upper
        ),
